    ...
    <div className="min-h-screen grid lg:grid-cols-5">
      <div className="hidden lg:flex lg:col-span-2 bg-linear-to-br from-indigo-500/10 via-background to-cyan-500/10 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="mb-8 h-64"><DiagnosticOrb /></div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-background/70 p-6 shadow-2xl shadow-indigo-500/10">
      ...
      <div className="flex items-center justify-center p-8 lg:col-span-3">
        <motion.div initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} className="w-full max-w-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-border bg-background/80 p-6 shadow-2xl shadow-indigo-500/10">
            ...
            <div>
              ...
              <Select value={role} onValueChange={setRole} disabled={isLoading}>
                <SelectTrigger id="role" className="mt-1"><SelectValue placeholder="Select account type" /></SelectTrigger>
                <SelectContent>
            ...
            {role === "COMPANY" && (
              <div className="grid gap-4 rounded-2xl border border-border bg-background/70 p-4 md:grid-cols-2">
                <div className="md:col-span-2"><Label htmlFor="companyName">Company name</Label><Input id="companyName" value={companyName} onChange={(event) => setCompanyName(event.target.value)} disabled={isLoading} className="mt-1" placeholder="Acme Diagnostics" /></div>
                <div><Label htmlFor="gstNumber">GST / Tax ID</Label><Input id="gstNumber" value={gstNumber} onChange={(event) => setGstNumber(event.target.value)} disabled={isLoading} className="mt-1" placeholder="Optional" /></div>
                <div><Label htmlFor="website">Website</Label><Input id="website" type="url" value={website} onChange={(event) => setWebsite(event.target.value)} disabled={isLoading} className="mt-1" placeholder="https://example.com" /></div>
              </div>
            )}
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-background/70 p-4 text-sm text-muted-foreground">
            <div className="flex items-center space-x-2"><UserRound className="h-4 w-4 text-cyan-400" /><span>Already have an account?</span></div>
          ...
          <div className="mt-6 rounded-2xl border border-border bg-background/70 p-4 text-sm text-muted-foreground">
            <div className="mb-2 flex items-center space-x-2"><Briefcase className="h-4 w-4 text-cyan-400" /><span className="font-semibold text-foreground">Why this works</span></div>
